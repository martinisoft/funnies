...
Given %r{^there is a (.+)$} do |model_name|
  create_from_table(model_name, {})
end
...
Given %r{^that (.+) has the following (.+):$} do |parent, child, table|
  ...
  if child.classify.constantize.method_defined?(parent.pluralize)
    ...
    if assoc.macro == :has_one
      parent_instance.send("#{assoc.name}=", @they.last)
    end
  else
    ...
    if assoc.present?
      ...
      raise "Explicit associaction does not exist between #{parent} and #{child}"
    end
  end
end
